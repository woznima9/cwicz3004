...
public class BalcJackPlay {
    public static void main(String[] args) {
        ...
        int key = 0;
        boolean nowaGra = true;
        int punktyGracza = 0;
        int punktyCasino = 0;
        talia.getTalia();
        talia.podejrzyjTalie();
        System.out.println("po inicjacji zerwoej");
        talia.potasujTalie();
        talia.podejrzyjTalie();
        while (nowaGra) {
            ...
            System.out.println("GRACZU Masz karty " + player.playerCards + getZdobytePunkty(punktyGracza) + "  grasz dalej? (Tak -1, Nie -0");
            ...
            while (decisionPlayera) {
                ...
                punktyGracza = player.obliczPunkty();
                if (punktyGracza == 21) {
                    System.out.println("Masz karty" + player.playerCards + getZdobytePunkty(punktyGracza));
                ...
                System.out.println("Masz karty" + player.playerCards + getZdobytePunkty(punktyGracza) + " grasz dalej? (Tak -1, Nie -0");
                decyzja = in.nextInt();
            ...
            if (punktyGracza <= 21) {
                ...
                System.out.println("KASYNO Masz karty " + casino.casinoCards + getZdobytePunkty(punktyCasino)+ " grasz dalej? (Tak -1, Nie -0");
                decyzja = in.nextInt();
                ...
                while (decisionCasina) {
                    ...
                    punktyCasino = casino.obliczPunkty();
                    if (punktyCasino == 21) {
                        System.out.println("Masz karty" + casino.casinoCards + getZdobytePunkty(punktyCasino));
                        System.out.println(" Brawo - trafiłeś Oczko ");
                        break;
                    }
                    if (punktyCasino > 21) {
                        System.out.println("KASYNO Masz karty " + casino.casinoCards + getZdobytePunkty(punktyCasino));
                ...
                if (punktyCasino <= 21) {
                    ...
                    if (punktyCasino == punktyGracza) {
                        ...
                        System.out.println("GRACZ  WYGRYWA  i zgarnia pulę ");
                    }
                }
            }

            System.out.println("\nCzy zaczynamy nową grę  Tak - 1  Nie -0");
            key = in.nextInt();
            nowaGra = (key == 1) ? true : false;
        }
    }

    private static String getZdobytePunkty(int punkty) {
        return " (zdobyte punkty " + punkty + ")";
    }
}
